...
class LocationBloc extends Bloc<LocationEvent, LocationState> {
  ...
  FutureOr<void> getUserCurrentLocation(
      GetUserCurrentLocation event, Emitter<LocationState> emit) async {
    ...
    try {
      ...
      emit(state.copyWith(
          position: currentPosition, isLoadingstatus: false, address: address));
    } catch (e) {
      emit(state.copyWith(
  ...
  FutureOr<void> updateCurrentLocation(
      UpdateCurrentLocation event, Emitter<LocationState> emit) async {
    String newAddress = await _getAddressFromLatLng(event.position);
    emit(
      state.copyWith(position: event.position, address: newAddress),
    );
  }
